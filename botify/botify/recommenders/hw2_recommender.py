...
class Solution(Recommender):
    def __init__(self, listen_history_redis, tracks_redis, catalog, fallback_recommender):
        ...
        self.total_listens = self._count_total_listens()
        self.next_threshold = self._get_next_threshold(self.total_listens)

    def _count_total_listens(self):
        keys = self.listen_history_redis.keys("user:*:listens")
        total = 0
        for key in keys:
            total += self.listen_history_redis.llen(key)
        return total

    def _get_next_threshold(self, current_total):
        for threshold in range(1000, 10001, 1000):
    ...
    def recommend_next(self, user: int, prev_track: int, prev_track_time: float) -> int:
        ...
        if self.next_threshold is not None and self.total_listens >= self.next_threshold:
            self._train()
            self.next_threshold = self._get_next_threshold(self.total_listens)
